...
@class TUCall; @class SpringBoard; @class NCNotificationRequest; @class SBRemoteAlertHandleServer; 
...
static id _logos_method$ringertimeslots$NCNotificationRequest$sound(_LOGOS_SELF_TYPE_NORMAL NCNotificationRequest* _LOGOS_SELF_CONST __unused self, SEL __unused _cmd) {
  if (![[self sectionIdentifier] isEqualToString:@"com.teststring.whatever"] && !checkDate()) {
    NSMutableDictionary *applist = [[NSMutableDictionary alloc] initWithContentsOfFile:@"/var/mobile/Library/Preferences/com.greg0109.callhiderapplist.plist"];
    for (NSString *selected in applist) {
      if ([[self  sectionIdentifier] isEqualToString:selected]) {
        return nil;
      }
    }
  }
